...
(function(){
  ...
  var nonCoverCount = SLIDES.filter(function(s){ return !s.image; }).length;
  var bgIdx = 0;
  function lerp(a,b,t){ return Math.round(a+(b-a)*t); }
  function slideColor(i,n){
    var t = n>1 ? i/(n-1) : 0;
    var start=[247,234,210], end=[214,138,69];
  ...
  SLIDES.forEach(function(s, i){
    ...
    if (s.image) d.style.backgroundImage = 'url(' + s.image + ')';
      ...
      d.style.background = 'linear-gradient(165deg, ' + slideColor(bgIdx, nonCoverCount) + ', var(--ink))';
      bgIdx++;
    }
