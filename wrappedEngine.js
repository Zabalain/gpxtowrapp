...
export function buildSlidesFromStats(stats, title, coverImage) {
  ...
  else distComment = `Más de ${fmt(stoppedMin / 60, 1)} horas paradas &mdash; esto tuvo pinta de excursión con calma, no de entrenamiento seguido.`;
  slides.push({
  ...
  // ---------- Potencia (con índice de variabilidad NP/media) ----------
  if (stats.hasPower) {
  ...
  if (stats.pedaling) {
    ...
    const parts = [];
    let correction = null; // true=recomendable revisar, false=todo normal, null=sin datos suficientes

    if (pd.leftBalancePct != null) {
      const dev = Math.abs(50 - pd.leftBalancePct);
      const dominant = pd.leftBalancePct > 50 ? 'izquierda' : 'derecha';
      if (dev <= 5) {
        parts.push(`Reparto muy equilibrado entre piernas (${fmt(pd.leftBalancePct, 0)}% / ${fmt(pd.rightBalancePct, 0)}%) &mdash; no hay nada que corregir ahí.`);
        correction = correction === true ? true : false;
      } else if (dev <= 10) {
        parts.push(`Ligera predominancia de la pierna ${dominant} (${fmt(pd.leftBalancePct, 0)}% / ${fmt(pd.rightBalancePct, 0)}%), dentro de lo habitual en la mayoría de ciclistas.`);
        correction = correction === true ? true : false;
      } else {
        parts.push(`Desequilibrio notable hacia la pierna ${dominant} (${fmt(pd.leftBalancePct, 0)}% / ${fmt(pd.rightBalancePct, 0)}%). Si se repite en más salidas, merece la pena revisar calas, altura de sillín o, si el medidor es de un solo lado, desconfiar del dato antes de sacar conclusiones.`);
        correction = true;
      }
    }
    ...
    if (pd.torqueEffLPct != null && pd.torqueEffRPct != null) {
      ...
      if (avgTe < 60) {
        parts.push(`Torque effectiveness bajo (izq. ${fmt(pd.torqueEffLPct, 0)}% / dcha. ${fmt(pd.torqueEffRPct, 0)}%): estás "tirando" de un lado del pedaleo más que empujando todo el círculo &mdash; trabajar el pedaleo redondo podría ayudarte.`);
        correction = true;
      } else {
      ...
      parts.push('El torque effectiveness venía a 0% en todos los puntos: el sensor no lo está reportando de verdad, así que no se puede evaluar &mdash; no es que tu pedaleo sea malo, es que falta el dato.');
    }
    ...
    slides.push({
      ...
      body: `<div class="sub" style="font-size:14.5px;max-width:38ch">${parts.join(' ')}</div>
  ...
  // ---------- Cadencia (cruzada con potencia si la hay) ----------
  if (stats.hasCadence) {
    let cadComment;
    if (stats.avgCadence == null) cadComment = '';
    else if (stats.avgCadence < 70) cadComment = stats.hasPower
      ? 'Cadencia baja con desarrollos largos &mdash; más fuerza por pedalada que velocidad de piernas.'
...
export function renderWrapped(container, slides, { onSave, onShare } = {}) {
  ...
  const bgs = ['#241a10', '#141f24', '#24141c', '#141f1c', '#241417', '#1c1f14', '#241f14', '#1c222c'];
  let bgIdx = 0;
